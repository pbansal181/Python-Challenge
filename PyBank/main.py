...
#iterate through the profits to find changes in profit/loss
 for i in range(len(total_profit) - 1):
    profit_change_month.append(total_profit[i+1] - total_profit[i])
...
#Final value of max incraese and decrease monthly
    max_inc_month = profit_change_month.index(max(profit_change_month)) + 1
    max_dec_month = profit_change_month.index(min(profit_change_month)) + 1
